...
class DetectSensitivity extends $pb.ProtobufEnum {
  static const DetectSensitivity SENSITIVITY_UNSPECIFIED =
      DetectSensitivity._(0, _omitEnumNames ? '' : 'SENSITIVITY_UNSPECIFIED');

  /// Detection threshold as configured.
  static const DetectSensitivity NORMAL =
      DetectSensitivity._(1, _omitEnumNames ? '' : 'NORMAL');

  /// Modestly lowered threshold.
  static const DetectSensitivity HIGH =
      DetectSensitivity._(2, _omitEnumNames ? '' : 'HIGH');

  /// Recovers the most stars but is prone to reporting noise or hot pixels as
  /// stars.
  static const DetectSensitivity HIGHEST =
      DetectSensitivity._(3, _omitEnumNames ? '' : 'HIGHEST');
  ...
  static final $core.List<DetectSensitivity?> _byValue =
      $pb.ProtobufEnum.$_initByValueList(values, 3);
  static DetectSensitivity? valueOf($core.int value) =>
      value < 0 || value >= _byValue.length ? null : _byValue[value];

  const DetectSensitivity._(super.value, super.name);
}
...
/// The orientation of the client's display. Deprecated; see below.
...
class ImageFormat extends $pb.ProtobufEnum {
  ...
  static const ImageFormat IMAGE_FORMAT_UNSPECIFIED =
      ImageFormat._(0, _omitEnumNames ? '' : 'IMAGE_FORMAT_UNSPECIFIED');
  static const ImageFormat JPEG =
      ImageFormat._(1, _omitEnumNames ? '' : 'JPEG');
